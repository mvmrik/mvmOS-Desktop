...
- Unread counts a page puts in its title — `(3) Inbox` — are shown next to the tab and added up for the whole app: a dock badge on macOS, a taskbar overlay on Windows, and on Linux both the launcher count and a tray icon with the number drawn into it, since a Linux panel is free to ignore the count and several do. The window is highlighted in the task list when something arrives while you are elsewhere, until you look at it.
