...
- Unread counts a page puts in its title — `(3) Inbox` — are shown next to the tab and added up for the whole app: a dock badge on macOS, a taskbar overlay on Windows, and on Linux the launcher count as well, since a Linux panel is free to ignore it and several do. The window is highlighted in the task list when something arrives while you are elsewhere, until you look at it.
- Linux and Windows also get a tray icon with the count drawn into it, which can be turned off, and can be told to keep the app running when the window is closed — the window hides instead, and comes back from the icon. macOS has neither: closing a window there already leaves the app in the dock with its badge.
...
- Settings live in the app menu and can lock the app with a PIN: while it is locked the window shows nothing at all, tabs and sidebar included. Once a PIN is set, the padlock in the sidebar header locks the window on the spot.
